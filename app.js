(function () {
  ...
  function buildLineChart(seriesList, meta, unit) {
    ...
    seriesList.forEach(function (s, sIdx) {
      ...
      pts.forEach(function (p, i) {
        ...
        var isAvg = p.n > 1;
        var ptLabel = fmtDateShort(p.x) + ": " + fmtNum(p.y) + " " + (unit || "") + " (" + s.label + ")" + (isAvg ? " · promedio de " + p.n + " muestras" : "");
        svg.push('<circle cx="' + xPos(p.x).toFixed(1) + '" cy="' + yPos(p.y).toFixed(1) + '" r="' + (isLast ? 4.5 : 3.4) + '" fill="var(--' + st + ')" stroke="' + color + '" stroke-width="1.6"' + (isAvg ? ' stroke-dasharray="1.6,1.4"' : "") + '><title>' + esc(ptLabel) + "</title></circle>");
        var labelY = labelAbove ? yPos(p.y) - 9 : yPos(p.y) + 17;
        var labelText = fmtNum(p.y) + (isAvg ? " (prom.)" : "");
        svg.push('<text x="' + xPos(p.x).toFixed(1) + '" y="' + labelY.toFixed(1) + '" text-anchor="middle" font-size="10.5" font-weight="' + (isLast ? 700 : 600) + '" fill="' + (isLast ? "var(--ink)" : color) + '"><title>' + esc(ptLabel) + "</title>" + esc(labelText) + "</text>");
      });
    });
  ...
  function renderCampoDetail(campo) {
    ...
    var tbodyHtml = tableRows.map(function (r) {
      ...
      var muestraCell = hasMuestra ? "<td>" + esc(r.muestra || "—") + "</td>" : "";
      return "<tr>" + fechaCell + informeCell + labCell + '<td><span class="tipo-pill ' + tipoClass(r.tipo) + '">' + esc(r.tipo || "—") + '</span></td><td>' + esc(r.prof || "—") + "</td>" + muestraCell + cells + scoreCell + "</tr>";
    }).join("");
